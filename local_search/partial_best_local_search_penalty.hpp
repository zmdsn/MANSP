 ...
 * 
 ************************************************************************/
#ifndef  __PaticalBestLocalSearchPenalty__HPP__
#define  __PaticalBestLocalSearchPenalty__HPP__
#include <sstream>
#include <iostream>
#include <string>
#include <ostream>
...
template<class M>
class PaticalBestLocalSearchPenalty:public LocalSearchBase{
  public:
    M* wanted_move;
    M* temp_move;
    double lamda;
    ...
    PaticalBestLocalSearchPenalty(CARP_INDIVIDUAL* indi):
      LocalSearchBase(indi){
      wanted_move = new M(indi);
      temp_move   = new M(indi);  
      set_lamda(0.1);
    };
    PaticalBestLocalSearchPenalty(CARP_INDIVIDUAL* indi,    M* _wanted_move,
      M* _move):LocalSearchBase(indi,_wanted_move,_move){
    }
    ~PaticalBestLocalSearchPenalty(){};
};
#endif
