 ...
 * 
 ************************************************************************/
#ifndef  __BestFitnessLocalSearch__HPP__
#define  __BestFitnessLocalSearch__HPP__
#include <sstream>
#include <iostream>
#include <string>
#include <ostream>
...
template<class M>
class BestFitnessLocalSearch:public LocalSearchBase{
  public:
    M* wanted_move;
    M* temp_move;
    void init_lamda() {
    ...
    BestFitnessLocalSearch(CARP_INDIVIDUAL* indi):LocalSearchBase(indi){
        wanted_move = new M(indi);
        temp_move   = new M(indi);  
    };
    BestFitnessLocalSearch(CARP_INDIVIDUAL* indi,    M* _wanted_move,
      M* _move):LocalSearchBase(indi,_wanted_move,_move){
    }
    ~BestFitnessLocalSearch(){};
};
#endif
